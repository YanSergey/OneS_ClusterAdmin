...
/** The dialog box shows the session information. */
public class SessionInfoDialog extends Dialog {
  ...
  public SessionInfoDialog(
      Shell parentShell, Server server, UUID clusterId, UUID sessionId, ISessionInfo sessionInfo) {
    super(parentShell);
  ...
  private void initInfobaseProperties() {
    ...
    var clientIpAddress =
        sessionInfo.getClientIPAddress() == null
            ? "" //$NON-NLS-1$
