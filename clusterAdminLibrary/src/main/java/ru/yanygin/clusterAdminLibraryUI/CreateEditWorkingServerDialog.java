...
//		parentShell.setText("Parameters of the 1C:Enterprise infobase");
	...
	private void initServerProperties() {
		...
		if (workingServerId != null) { // –едактируем существующий рабочий сервер
		...
		} else { // —оздаем новый рабочий сервер
		...
		if (workingServerId != null) { // ” уже созданного кластера запрещено мен€ть хост и порт
			txtServerName.setEditable(false);
			txtComputerName.setEditable(false);
			txtIPPort.setEditable(false);
		} else {
			btnIsMainServer.setEnabled(false); // ѕочему новому серверу запрещено сразу ставить галочку ÷ентральный сервер?
